...
| `enableASM`                   | **Deprecated**: use `enableAppsec` or `enableAppsecRuntimeAPIProxy` instead. Enable [Datadog App and API Protection (AAP)][19] on the Lambda function. Requires tracing to be enabled (`enableDDTracing`). Defaults to `false`.                                                                                                                                                                                                                                                                                                                                                          |
| `enableAppsec`                | Enable [Datadog App and API Protection (AAP)][19] on the Lambda function using the recommended env var for your runtime. For Python runtimes, if you are manually installing the [Datadog Lambda Library][8] ensure that you are using a version that satifies `>=8.114.0`. Requires tracing to be enabled (`enableDDTracing`). Defaults to `false`.                                                               |
| `enableAppsecRuntimeApiProxy` | Enable [Datadog App and API Protection (AAP)][19] on the Lambda function using a Runtime API Proxy in the Lambda extension. Requires tracing to be enabled (`enableDDTracing`). Defaults to `false`.                                                                                                                                                         |
