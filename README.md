...
| `forwarderArn`          | Setting this parameter subscribes the given Datadog forwarder to the Lambda functions’ CloudWatch log groups. Required when `enableDDTracing` is set to `true` unless the subscription is otherwise applied. If a subscription is applied via an integration such as DataDogs AWS Integration, this is not required. Do not set `addExtension` to `true` when using `forwarderArn`.                                                                                                                                                                                                                 |
