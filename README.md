...
| `apiKMSKey`                 | Datadog API Key encrypted using KMS. Use this parameter if you're storing your secrets via KMS, and if `flushMetricsToLogs` is `false` or `addExtension` is `true`. Defining `apiKMSKey` will add the Datadog API Key directly to your Lambda functions as an environment variable.                                                                                                                                                      |
