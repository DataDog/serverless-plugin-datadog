...
| `enableAppsec`                | Enable [Datadog App and API Protection (AAP)][19] on the Lambda function using the recommended env var for your runtime. For Python runtimes, if you are manually installing the [Datadog Lambda Library][8], ensure that you are using version `8.114.0` or later. Requires tracing to be enabled (`enableDDTracing`). Defaults to `false`.                                                               |
