...
| `isFIPSEnabled`    |  When set to `true`, a FIPS-compliant Lambda extension layer is used. This only works if `addExtension` is `true`, and `site` is `ddog-gov.com`. Defaults to `true` if `addExtension` is `true`, and AWS region starts with `us-gov-`. Defaults to `false` otherwise.                                                                                                                                                                  |
