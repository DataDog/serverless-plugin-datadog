...
// afterEach(() => {
//     (buildMonitorParams as unknown as jest.Mock).mockClear();
// })
...
describe("setMonitors", () => {
    ...
    it("returns an array of updated monitors, an array of created monitors, an array of deleted monitors", async () => {
        // const monitorParams = { "name": "Custom Monitor 1", "options": {}, "query": "max(next_1w):forecast(avg:system.load.1{*}, 'linear', 1, interval='60m', history='1w', model='default') >= 3", "tags": ["serverless_monitor_type:single_function", "serverless_monitor_id:custom_monitor_1", "aws_cloudformation_stack-id:cloud_formation_id", "created_by:dd_sls_plugin", "env:env", "service:service"], "type": "metric alert" };
        ...
        const logStatements = await setMonitors([CUSTOM_MONITOR_1], "apikey", "appkey", "cloud_formation_id", "service", "env")
        ...
        //expect(createMonitor as unknown as jest.Mock).toHaveBeenCalledWith({ "name": "Custom Monitor 1", "options": {}, "query": "max(next_1w):forecast(avg:system.load.1{*}, 'linear', 1, interval='60m', history='1w', model='default') >= 3", "tags": ["serverless_monitor_type:single_function", "serverless_monitor_id:custom_monitor_1", "aws_cloudformation_stack-id:cloud_formation_id", "created_by:dd_sls_plugin", "env:env", "service:service"], "type": "metric alert" }, "custom_monitor_1", "apikey", "appkey");
    })
    it("returns an array of updated monitors, an array of created monitors, an array of deleted monitors", async () => {
        ...
        expect(logStatements).toEqual(["Successfully updated custom_monitor_1", "Successfully created custom_monitor_2, increased_cost"]);
        expect(updateMonitor as unknown as jest.Mock).toHaveBeenCalledWith(123456, "custom_monitor_1", CUSTOM_MONITOR_1_PARAMS, "apikey", "appkey");
    ...
    it("returns an array of updatedmonitors, an array of created monitors, an array of deleted monitors", async () => {
        (getExistingMonitors as unknown as jest.Mock).mockReturnValue({ "custom_monitor_1": 123456, "custom_monitor_2": 123456, "increased_cost": 123456 });
        (createMonitor as unknown as jest.Mock).mockReturnValue({ status: 200 });
        (updateMonitor as unknown as jest.Mock).mockReturnValue({ status: 200 });
        ...
        expect(logStatements).toEqual(["Successfully updated custom_monitor_1, custom_monitor_2", "Successfully created timeout", "Successfully deleted increased_cost"]);
        expect(updateMonitor as unknown as jest.Mock).toHaveBeenCalledWith(123456, "custom_monitor_1", CUSTOM_MONITOR_1_PARAMS, "apikey", "appkey");
        expect(updateMonitor as unknown as jest.Mock).toHaveBeenCalledWith(123456, "custom_monitor_2", UPDATED_CUSTOM_MONITOR_2_PARAMS, "apikey", "appkey"); //make sure to use the UPDATED_Monitors? 
    ...
    it("returns an array of updated monitors, an array of created monitors, an array of deleted monitors", async () => {
        (getExistingMonitors as unknown as jest.Mock).mockReturnValue({ "timeout": 123456, "custom_monitor_1": 123456, "custom_monitor_2": 123456 });
        (createMonitor as unknown as jest.Mock).mockReturnValue({ status: 200 });
        (updateMonitor as unknown as jest.Mock).mockReturnValue({ status: 200 });
        (deleteMonitor as unknown as jest.Mock).mockReturnValue({ status: 200 });
        const logStatements = await setMonitors(MONITOR_SET_3, "apikey", "appkey", "cloud_formation_id", "service", "env");
        expect(logStatements).toEqual(["Successfully updated custom_monitor_1", "Successfully created increased_cost", "Successfully deleted timeout, custom_monitor_2"]);
        expect(updateMonitor as unknown as jest.Mock).toHaveBeenCalledWith(123456, "custom_monitor_1", CUSTOM_MONITOR_1_PARAMS, "apikey", "appkey");
