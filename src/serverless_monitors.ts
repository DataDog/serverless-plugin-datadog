...
export const HIGH_ERROR_RATE: ServerlessMonitor = {
    ...
    query: (cloudFormationStackId: string) => {
        return `avg(last_15m):sum:aws.lambda.errors{aws_cloudformation_stack-id:${cloudFormationStackId}} by {functionname,region,aws_account}.as_count() / sum:aws.lambda.invocations{aws_cloudformation_stack-id:${cloudFormationStackId}} by {functionname,region,aws_account}.as_count() >= 0.1`
    },
    // query:
    //     `avg(last_15m):sum:aws.lambda.errors{service:${process.env.SERVICE}} by {functionname,region,aws_account}.as_count() / sum:aws.lambda.invocations{*} by {functionname,region,aws_account}.as_count() >= 0.1`,
    threshold: 0.1,
...
export const TIMEOUT: ServerlessMonitor = {
    ...
    query: (cloudFormationStackId: string) => {
        return `avg(last_15m):sum:aws.lambda.duration.maximum{aws_cloudformation_stack-id:${cloudFormationStackId}} by {aws_account,functionname,region}.as_count() / (sum:aws.lambda.timeout{aws_cloudformation_stack-id:${cloudFormationStackId}} by {aws_account,functionname,region}.as_count() * 1000) >= 1`
    },
    // query:
    //     `avg(last_15m):sum:aws.lambda.duration.maximum{service:${process.env.SERVICE}} by {aws_account,functionname,region}.as_count() / (sum:aws.lambda.timeout{*} by {aws_account,functionname,region}.as_count() * 1000) >= 1`,
    threshold: 1,
...
export const OUT_OF_MEMORY: ServerlessMonitor = {
    ...
    query: (cloudFormationStackId: string) => {
        return `avg(last_15m):sum:aws.lambda.enhanced.max_memory_used{aws_cloudformation_stack-id:${cloudFormationStackId}} by {aws_account,functionname,region}.as_count() / sum:aws.lambda.memorysize{aws_cloudformation_stack-id:${cloudFormationStackId}} by {aws_account,functionname,region}.as_count() >= 1`
    },
    // query:
    //     `avg(last_15m):sum:aws.lambda.enhanced.max_memory_used{service:${process.env.SERVICE}} by {aws_account,functionname,region}.as_count() / sum:aws.lambda.memorysize{*} by {aws_account,functionname,region}.as_count() >= 1`,
    threshold: 1,
...
export const HIGH_ITERATOR_AGE: ServerlessMonitor = {
    ...
    // query: `avg(last_15m):min:aws.lambda.iterator_age.maximum{service:${process.env.SERVICE}} by {aws_account,region,functionname} >= 86400`,
    threshold: 86400,
...
export const HIGH_COLD_START_RATE: ServerlessMonitor = {
    ...
    query: (cloudFormationStackId: string) => {
        return `avg(last_15m):sum:aws.lambda.enhanced.invocations{cold_start:true AND aws_cloudformation_stack-id:${cloudFormationStackId}} by {aws_account,functionname,region}.as_count() / sum:aws.lambda.enhanced.invocations{aws_cloudformation_stack-id:${cloudFormationStackId}} by {aws_account,functionname,region}.as_count() >= 0.2`
    },
    // query:
    //     `avg(last_15m):sum:aws.lambda.enhanced.invocations{cold_start:true AND service:${process.env.SERVICE}} by {aws_account,functionname,region}.as_count() / sum:aws.lambda.enhanced.invocations{service: ${process.env.SERVICE}} by {aws_account,functionname,region}.as_count() >= 0.2`,
    threshold: 0.2,
...
export const HIGH_THROTTLES: ServerlessMonitor = {
    ...
    query: (cloudFormationStackId: string) => {
        return `avg(last_15m):sum:aws.lambda.throttles {aws_cloudformation_stack-id:${cloudFormationStackId}} by {aws_account,region,functionname}.as_count() / sum:aws.lambda.invocations{aws_cloudformation_stack-id:${cloudFormationStackId}} by {aws_account,region,functionname}.as_count() >= 0.2`
    },
    // query:
    //     `avg(last_15m):sum:aws.lambda.throttles {service:${process.env.SERVICE}} by {aws_account,region,functionname}.as_count() / sum:aws.lambda.invocations{service: ${process.env.SERVICE}} by {aws_account,region,functionname}.as_count() >= 0.2`,
    threshold: 0.2,
...
export const INCREASED_COST: ServerlessMonitor = {
    ...
    query: (cloudFormationStackId: string) => {
        return `pct_change(avg(last_5m),last_5m):avg:aws.lambda.enhanced.estimated_cost{aws_cloudformation_stack-id:${cloudFormationStackId}} > 20`
    },
    // query:
    //     `pct_change(avg(last_5m),last_5m):avg:aws.lambda.enhanced.estimated_cost{service:${process.env.SERVICE}} > 20`,
    threshold: 0.2,
